...
class FavProduct extends StatefulWidget {
  FavProduct({Key? key, this.storeName, this.Id, this.isFav}) : super(key: key);
  var storeName;
  var Id;
  var isFav;
...
class _StoreItemDetail extends State<FavProduct> {
  var dataLoad = false;
  var STORES = "";
  Map? myLang;
  var dataFound = false;
...
/*    Map myLangData = await langTxt(context);
  ...
  */
  ...
  void callUSerStatusCheck() async {
    ...
    Map decoded =
        jsonDecode(await apiRequest(favoriteProductUrl, map, context));
...
//    _showToast(context, message);
  ...
  Future<bool?> getDetail(
      bool isLiked, String store_id, String product_id) async {
    var callUserLoginCheck = await internetConnectionState();
    if (callUserLoginCheck == true) {
/*      Map map = {
        "store_id": store_id,
      ...
      var api = favUnfavoriteProductUrl + product_id;
      ...
      if (status == success_status) {
        ...
        Map decoded = jsonDecode(await apiRefreshRequest(context));
        getDetail(isLiked, store_id, product_id);
      } else {
        _showToast(message);
        return false;
      }
...
//like N or something
  ...
  void setData(categoryIdSelected, category, minPrice, maxPrice) {
    ...
    if (category != '' || minPrice != '' || maxPrice != '') validationCheck();
  }
